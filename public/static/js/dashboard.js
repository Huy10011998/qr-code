(function ($) {
  ...
  $(
    "#username__, #fullName, #fullName_en,#password, #department,#department_en, #userId, #email, #inputImportImage, #phoneNumber, #roles"
  ).on("input", function () {
    checkInputsCreate();
  });
  ...
  function initializeInputValues() {
    ...
    inputs.forEach(function (input) {
      inputValues[input] = $("#" + input)
        .val()
        .trim(); // Lưu giá trị ban đầu vào state
    });
  }
  ...
  tableEl.on("click", ".img-delete-qrcode", function (event) {
    ...
    contentDelete.html(
      `Bạn chắc chắn muốn xoá mã Qr Code của tài khoản: ${userId}`
    );
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.code === 200) {
          ...
          modalDelete.css("display", "none");
          body.css("overflow", "auto");
          customToastify("Xoá mã QR không thành công!", {
            background: BG_TOAST[2],
          });
        }
      },
      error: function (xhr) {
        customToastify("Hệ thống đang bận! Vui lòng thử lại sau!", {
          background: BG_TOAST[2],
        });
      },
      complete: function (xhr) {},
    ...
    $.ajax({
      ...
      success: function (response) {
        if (+response.code === 200) {
          ...
          customToastify("Cập nhật mã QR thành công!", {
            background: BG_TOAST[0],
          });
        }
      },
      error: function (xhr, status, error) {
  ...
  function exportToExcel() {
    ...
    Array.from(rows).forEach((row, rowIndex) => {
      if (rowIndex > 0) {
        ...
        if (cells.length > 0) {
          const checkbox = cells[18].querySelector('input[type="checkbox"]');
          if (!checkbox) {
            return;
          }
        ...
        function (blob) {
          ...
          reader.onloadend = function () {
            ...
            image.onload = function () {
              ...
              compressedDataURLUpdate = compressedCanvas.toDataURL(
                "image/jpeg",
                0.6
              );
            };
  ...
  $("#btn-img-save-update").on("click", function () {
    ...
    image.onload = function () {
      ...
      $("#show-image-update").html(
        '<img src="' + compressedDataURLUpdate + '">'
      );
